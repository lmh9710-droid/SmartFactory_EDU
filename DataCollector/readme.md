...
1. 🌡️ **임베디드 센서 제어:** Renesas/AVR 마이크로컨트롤러 환경에서 온습도(DHS) 및 조도(CDS) 센서 데이터를 주기적으로 획득하는 펌웨어 개발
2. ⚡ **C++ 미들웨어 데이터 수집기:** UART 통신으로 수신한 센서 스트림을 읽어 SQLite DB에 비동기/안전하게 적재하는 멀티스레드 수집기 작성
3. 📊 **웹 대시보드 모니터링:** SQLite DB 데이터를 실시간 연동하여 공정별 센서 수치를 시각화하는 Streamlit 대시보드 구축
...
| 구분 | 장비 / 부품명 | 이미지 / 설명 |
| :--- | :--- | :--- |
| **MCU 1** | **R7FA4M1AB3CFM (Renesas)** | 32-bit ARM® Cortex®-M4 코어 기반 메인 프로세서 |
| **MCU 2** | **ATmega328P** | 8-bit AVR® 마이크로컨트롤러 |
| **Sensor 1** | **DHS Sensor** | 🌡️ 실시간 온습도 측정 센서 |
| **Sensor 2** | **CDS Sensor** | 💡 공정 내 광도(조도) 측정 센서 |
| **Interface** | **USB Cable** | 🔌 USB Type-C & USB A to B (UART-to-Serial) |
...
graph LR
    ...
    subgraph MW ["⚙️ Middleware Engine"]
        C[UART Serial Port] -->|Data Stream| D[C++ Multi-Thread Collector]
    end

    subgraph DB ["🗄️ Storage & UI"]
        E[(SQLite DB)] -->|Query| F[Python Streamlit Dashboard]
    end
...
sequenceDiagram
    ...
    participant MW as ⚙️ C++ 미들웨어 (Thread)
    ...
    loop 실시간 모니터링
        UI->>DB: 공정별 최신 계측 데이터 조회 (SELECT)
        DB-->>UI: 센서 계측치 데이터 반환
        UI->>UI: 시계열 트렌드 차트 및 UI 갱신
    end
